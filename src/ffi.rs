...
use crate::{VecLite, VecLiteConfig};
...
/// Opaque handle for VecLite instance
...
unsafe fn c_char_to_string(c_str: *const c_char) -> Result<String, FFIErrorCode> {
    if c_str.is_null() {
...
unsafe fn ffi_vector_to_vector_data(ffi_vec: FFIVector) -> Result<VectorData, FFIErrorCode> {
...
unsafe fn ffi_metadata_to_metadata(ffi_meta: FFIMetadata) -> Result<Metadata, FFIErrorCode> {
...
#[no_mangle]
pub extern "C" fn veclite_new() -> VecLiteHandle {
    match VecLite::new() {
...
/// Create a new VecLite instance with custom configuration
#[no_mangle]
pub extern "C" fn veclite_new_with_config(max_vectors: c_uint, default_k: c_uint) -> VecLiteHandle {
    ...
    match VecLite::with_config(config) {
        Ok(db) => Box::into_raw(Box::new(db)) as VecLiteHandle,
        Err(_) => ptr::null_mut(),
    }
}

/// Free a VecLite instance
#[no_mangle]
pub extern "C" fn veclite_free(handle: VecLiteHandle) {
    if !handle.is_null() {
        unsafe {
            let _db = Box::from_raw(handle as *mut VecLite);
...
pub extern "C" fn veclite_insert(
    ...
    unsafe {
        let db = &*(handle as *const VecLite);
...
#[no_mangle]
pub extern "C" fn veclite_delete(handle: VecLiteHandle, id: *const c_char) -> FFIResult {
    if handle.is_null() {
        return FFIResult::error(FFIErrorCode::NullPointer, "VecLite handle is null");
    }
...
#[no_mangle]
pub extern "C" fn veclite_len(handle: VecLiteHandle) -> c_uint {
    if handle.is_null() {
        return 0;
    }
...
#[no_mangle]
pub extern "C" fn veclite_is_empty(handle: VecLiteHandle) -> c_int {
    if handle.is_null() {
...
#[no_mangle]
pub extern "C" fn veclite_save(handle: VecLiteHandle, path: *const c_char) -> FFIResult {
    if handle.is_null() {
        return FFIResult::error(FFIErrorCode::NullPointer, "VecLite handle is null");
    }
...
#[no_mangle]
pub extern "C" fn veclite_load(path: *const c_char) -> VecLiteHandle {
    unsafe {
        let file_path = match c_char_to_string(path) {
            Ok(s) => s,
...
#[no_mangle]
pub extern "C" fn veclite_free_string(s: *mut c_char) {
    if !s.is_null() {
        unsafe {
...
/// Free search results
#[no_mangle]
pub extern "C" fn veclite_free_search_results(results: *mut FFISearchResults) {
    if results.is_null() {
        return;
    }
    ...
    unsafe {
        ...
        if !results_ref.results.is_null() {
            let results_slice =
                slice::from_raw_parts_mut(results_ref.results, results_ref.len as usize);
            ...
            for result in &mut *results_slice {
                ...
                if !result.metadata.entries.is_null() {
                    let metadata_slice = slice::from_raw_parts_mut(
                    ...
                    let _ = Box::from_raw(metadata_slice.as_mut_ptr());
                }
            }

            let _ = Box::from_raw(results_slice.as_mut_ptr());
        }
...
#[no_mangle]
pub extern "C" fn veclite_get_available_metrics(
    ...
    let available = VecLite::available_metrics();
